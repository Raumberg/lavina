// Stage 11: string interpolation, bare enum variants, ast module extraction, parenthesization fix
#include "lavina.h"
...
struct CppCodegen {
    ...
    bool type_contains_dynamic(TypeNode t) {
        {
            auto& _match_0 = t;
            if (_match_0._tag == "Dynamic") {
                return true;
            }
            else if (_match_0._tag == "Array") {
    ...
    std::string emit_type(TypeNode t) {
        {
            ...
            else if (_match_1._tag == "Nullable") {
    ...
    std::string emit_expr(Expr e) {
        {
            ...
            else if (_match_2._tag == "IndexSet") {
    ...
    std::string emit_call_expr(Expr callee, std::vector<Expr> args, bool in_method) {
        {
            ...
            else if (_match_4._tag == "StaticGet") {
            ...
            else if (_match_4._tag == "Variable") {
    ...
    std::string emit_method_expr(Expr e) {
        {
            ...
            else if (_match_6._tag == "IndexSet") {
    ...
    void emit_stmt(Stmt s) {
        {
            ...
            else if (_match_10._tag == "ExprStmt") {
            ...
            else if (_match_10._tag == "Function") {
            ...
            else if (_match_10._tag == "Namespace") {
    ...
    void emit_class(Token name, std::vector<Stmt> body) {
        ...
        if (has_init) {
            ...
            for (auto& st : init_body) {
                {
                    auto& _match_23 = st;
                    if (_match_23._tag == "ExprStmt") {
                        auto& _v = std::get<Stmt::ExprStmt>(_match_23._data);
                        auto& expr = _v.expr;
                        {
                            auto& _match_24 = expr;
                            if (_match_24._tag == "Set") {
        ...
        if (has_init) {
            ...
            for (auto& st : init_body) {
                {
                    auto& _match_26 = st;
                    if (_match_26._tag == "ExprStmt") {
                        auto& _v = std::get<Stmt::ExprStmt>(_match_26._data);
                        auto& expr = _v.expr;
                        bool handled = false;
                        {
                            auto& _match_27 = expr;
                            if (_match_27._tag == "Set") {
    ...
    std::string infer_expr_type(Expr e, std::vector<std::string> param_names, std::vector<std::string> param_types) {
        {
            ...
            else if (_match_32._tag == "Variable") {
    ...
    void emit_enum(Token name, std::vector<EnumVariantNode> variants) {
        ...
        for (auto& v : variants) {
            for (auto& ft : v.types) {
                {
                    auto& _match_33 = ft;
                    if (_match_33._tag == "Custom") {
                        auto& _v = std::get<TypeNode::Custom>(_match_33._data);
                        auto& n = _v.name;
                        if ((n == enum_name)) {
                            has_self_ref = true;
                        }
        ...
        for (auto& v : variants) {
            if ((static_cast<int64_t>(v.types.size()) > 0LL)) {
                ...
                while ((fi < static_cast<int64_t>(v.types.size()))) {
                    ...
                    {
                        auto& _match_34 = v.types[fi];
                        if (_match_34._tag == "Custom") {
        ...
        if ((static_cast<int64_t>(variant_inner_types.size()) > 0LL)) {
            this->output = (((((this->output + std::string("")) + ((*this).indent())) + std::string("std::variant<std::monostate, ")) + (lv_join(variant_inner_types, std::string(", ")))) + std::string("> _data;\n"));
        }
        this->output = (this->output + std::string("\n"));
        for (auto& v : variants) {
            ...
            if ((static_cast<int64_t>(v.types.size()) == 0LL)) {
                ...
                this->output = (((((((((((this->output + std::string("")) + ((*this).indent())) + std::string("static ")) + (enum_name)) + std::string(" make_")) + (vname)) + std::string("() { return {\"")) + (vname)) + std::string("\"")) + (mono)) + std::string("}; }\n"));
            }
             else {
                        ...
                        while ((bi < static_cast<int64_t>(arm.bindings.size()))) {
                            if ((bi < static_cast<int64_t>(vinfo.field_names.size()))) {
                                ...
                                if ((bi < static_cast<int64_t>(vinfo.types.size()))) {
                                    {
                                        auto& _match_36 = vinfo.types[bi];
                                        if (_match_36._tag == "Custom") {
    ...
    void emit_arm_body(Stmt arm_body, bool in_method) {
        {
            auto& _match_37 = arm_body;
            if (_match_37._tag == "Block") {
                auto& _v = std::get<Stmt::Block>(_match_37._data);
                auto& stmts = _v.statements;
                for (auto& st : stmts) {
                    if (in_method) {
                        (*this).emit_method_stmt(st);
    ...
    std::string generate(std::vector<Stmt> stmts) {
        ...
        for (auto& stmt : stmts) {
            {
                auto& _match_38 = stmt;
                if (_match_38._tag == "Function") {
                    auto& _v = std::get<Stmt::Function>(_match_38._data);
                    auto& name = _v.name;
                    auto& params = _v.params;
                    auto& return_type = _v.return_type;
                    auto& body = _v.body;
...
struct Parser {
    ...
    Expr assignment() {
        ...
        if ((*this).match_any(std::vector{TK_EQUAL})) {
            ...
            {
                auto& _match_39 = expr;
                if (_match_39._tag == "Variable") {
                    auto& _v = std::get<Expr::Variable>(_match_39._data);
                    auto& name = _v.name;
                    return Expr::make_Assign(name, value);
                }
                else if (_match_39._tag == "Get") {
                    auto& _v = std::get<Expr::Get>(_match_39._data);
                    auto& object = *_v.object;
                    auto& name = _v.name;
                    return Expr::make_Set(object, name, value);
                }
                else if (_match_39._tag == "Index") {
                    auto& _v = std::get<Expr::Index>(_match_39._data);
                    auto& object = *_v.object;
                    auto& bracket = _v.bracket;
                    auto& index = *_v.index;
    ...
    Expr finish_call(Expr callee) {
        {
            auto& _match_40 = callee;
            if (_match_40._tag == "Variable") {
...
void test_parser() {
    ...
    for (auto& stmt : stmts) {
        {
            auto& _match_41 = stmt;
            if (_match_41._tag == "ExprStmt") {
                auto& _v = std::get<Stmt::ExprStmt>(_match_41._data);
                auto& expr = _v.expr;
                {
                    auto& _match_42 = expr;
                    if (_match_42._tag == "Binary") {
                        auto& _v = std::get<Expr::Binary>(_match_42._data);
                        auto& left = *_v.left;
                        auto& op = _v.op;
                        auto& right = *_v.right;
                        print((std::string("Binary: ") + op.lexeme));
                        {
                            auto& _match_43 = right;
                            if (_match_43._tag == "Binary") {
                                auto& _v = std::get<Expr::Binary>(_match_43._data);
                                auto& rl = *_v.left;
                                auto& rop = _v.op;
                                auto& rr = *_v.right;
