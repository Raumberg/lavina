            ...
            if (std::holds_alternative<Expr::Literal>(_match_34._data)) {
                ...
                auto& args = _v.args;
                {
                    ...
                    if (std::holds_alternative<Expr::Variable>(_match_35._data)) {
                        ...
                        int64_t ci = 0LL;
                        while ((ci < static_cast<int64_t>(param_names.size()))) {
            ...
            if ((*this).match_any(std::vector{TK_NEWLINE})) {
                ...
                if ((*this).check(TK_IDENTIFIER) && ((*this).peek_at(1LL).token_type == TK_LEFT_PAREN)) {
                    ...
                    if ((*this).check(TK_IDENTIFIER) && (((*this).peek_at(1LL).token_type == TK_NEWLINE) || ((*this).peek_at(1LL).token_type == TK_DEDENT))) {
                        ...
                        {
                            auto& _match_44 = vtype;
                            if (std::holds_alternative<TypeNode::NullType>(_match_44._data)) {
                                is_unit_type = true;
                            }
                            else if (std::holds_alternative<TypeNode::Void>(_match_44._data)) {
                                is_unit_type = true;
                            }
                            else {
                                int64_t noop = 0LL;
...
void test_parser() {
    ...
    for (auto& stmt : stmts) {
        {
            auto& _match_45 = stmt;
            if (std::holds_alternative<Stmt::ExprStmt>(_match_45._data)) {
                auto& _v = std::get<Stmt::ExprStmt>(_match_45._data);
                auto& expr = _v.expr;
                {
                    auto& _match_46 = expr;
                    if (std::holds_alternative<Expr::Binary>(_match_46._data)) {
                        auto& _v = std::get<Expr::Binary>(_match_46._data);
                        auto& left = *_v.left;
                        auto& op = _v.op;
                        auto& right = *_v.right;
                        print((std::string("Binary: ") + op.lexeme));
                        {
                            auto& _match_47 = right;
