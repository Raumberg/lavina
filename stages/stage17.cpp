...
struct CppCodegen {
        ...
        for (const auto& v : variants) {
            if ((static_cast<int64_t>(v.types.size()) > 0LL)) {
                ...
                for (int64_t fi = 0LL; fi < static_cast<int64_t>(v.types.size()); fi++) {
                    ...
                    {
                        const auto& _match_43 = v.types[fi];
    ...
    void emit_enum_operators(const std::string& enum_name, const std::string& type_suffix, const std::string& tmpl_line) {
    ...
    TypeNode infer_type(const Expr& e) {
        {
            ...
            else if (std::holds_alternative<std::decay_t<decltype(_match_69)>::AddressOf>(_match_69._data)) {
                auto& _v = std::get<std::decay_t<decltype(_match_69)>::AddressOf>(_match_69._data);
                auto& expr = *_v.expr;
