...
struct Parser {
    ...
    std::string type_to_string(const TypeNode& t) {
        {
            ...
            if (std::holds_alternative<TypeNode::Int>(_match_64._data)) {
                return std::string("int64_t");
            }
            else if (std::holds_alternative<TypeNode::Float>(_match_64._data)) {
                return std::string("double");
            }
            else if (std::holds_alternative<TypeNode::Str>(_match_64._data)) {
                return std::string("std::string");
            }
            else if (std::holds_alternative<TypeNode::Bool>(_match_64._data)) {
                return std::string("bool");
            }
            else if (std::holds_alternative<TypeNode::Void>(_match_64._data)) {
                return std::string("void");
            }
            else if (std::holds_alternative<TypeNode::Auto>(_match_64._data)) {
                return std::string("auto");
            }
            else if (std::holds_alternative<TypeNode::Custom>(_match_64._data)) {
                auto& _v = std::get<TypeNode::Custom>(_match_64._data);
                auto& name = _v.name;
                auto& type_args = _v.type_args;
                if ((static_cast<int64_t>(type_args.size()) > 0LL)) {
                    std::vector<std::string> ta = {};
            ...
            else if (std::holds_alternative<TypeNode::Array>(_match_64._data)) {
                auto& _v = std::get<TypeNode::Array>(_match_64._data);
                auto& inner = *_v.inner;
        ...
        if ((*this).match_any(std::vector{TK_EQUAL})) {
            ...
            if ((*this).match_any(std::vector{TK_PLUS_EQUAL, TK_MINUS_EQUAL, TK_STAR_EQUAL})) {
                ...
                auto op_token = Token(base_type, base_lexeme, compound_op.line, compound_op.col);
                {
                    ...
                    else if (std::holds_alternative<Expr::Get>(_match_66._data)) {
                        auto& _v = std::get<Expr::Get>(_match_66._data);
                        auto& object = *_v.object;
                        auto& name = _v.name;
                        Expr bin = Expr::make_Binary(Expr::make_Get(object, name), op_token, rhs);
                        return Expr::make_Set(object, name, bin);
                    }
                    else if (std::holds_alternative<Expr::Index>(_match_66._data)) {
                        auto& _v = std::get<Expr::Index>(_match_66._data);
                        auto& object = *_v.object;
                        auto& bracket = _v.bracket;
                        auto& index = *_v.index;
    ...
    Expr call() {
        Expr expr = (*this).primary();
        {
            ...
            if (std::holds_alternative<Expr::Variable>(_match_67._data)) {
                auto& _v = std::get<Expr::Variable>(_match_67._data);
                auto& tok = _v.name;
                if ((*this).check(TK_LEFT_BRACKET)) {
                    int64_t save_pos = this->current;
                    (*this).advance();
    ...
    Expr finish_call(const Expr& callee) {
        {
            ...
            if (std::holds_alternative<Expr::Variable>(_match_68._data)) {
                auto& _v = std::get<Expr::Variable>(_match_68._data);
                auto& name = _v.name;
                ...
                if ((*this).check(TK_IDENTIFIER) && ((*this).peek_at(1LL).token_type == TK_LEFT_PAREN)) {
                    ...
                    if ((*this).check(TK_IDENTIFIER) && (((*this).peek_at(1LL).token_type == TK_NEWLINE) || ((*this).peek_at(1LL).token_type == TK_DEDENT))) {
                        ...
                        bool is_unit_type = false;
                        {
                            ...
                            if (std::holds_alternative<TypeNode::NullType>(_match_69._data)) {
                                is_unit_type = true;
                            }
                            else if (std::holds_alternative<TypeNode::Void>(_match_69._data)) {
                                is_unit_type = true;
                            }
                            else {
                                /* pass */
    ...
    for (const auto& stmt : stmts) {
        {
            ...
            if (std::holds_alternative<Stmt::ExprStmt>(_match_70._data)) {
                auto& _v = std::get<Stmt::ExprStmt>(_match_70._data);
                auto& expr = _v.expr;
                {
                    const auto& _match_71 = expr;
                    if (std::holds_alternative<Expr::Binary>(_match_71._data)) {
                        auto& _v = std::get<Expr::Binary>(_match_71._data);
                        auto& left = *_v.left;
                        auto& op = _v.op;
                        auto& right = *_v.right;
                        print((std::string("Binary: ") + op.lexeme));
                        {
                            ...
                            if (std::holds_alternative<Expr::Binary>(_match_72._data)) {
                                auto& _v = std::get<Expr::Binary>(_match_72._data);
                                auto& rl = *_v.left;
                                auto& rop = _v.op;
                                auto& rr = *_v.right;
